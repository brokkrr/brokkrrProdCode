...
function pageBeforeLoad(webServiceUrlfromView, BrokerMessageId1, CustomerMessageId1, BrokerId1, CustomerId1, CustomerName1, MainMessage1) {
    ...
    $.ajax({
        type: "POST",
        url: webServiceUrl,
        data: { UserId: BrokerId, MessageId: BrokerMessageId, ActionName: "DoGetUnreadChatMessages" },
                ...
                $.each(chatMessage, function (i, res) {
                    ...
                    if (res.BrokerMessage != '') {
                        ...
                        $('#chat1').append('<a id="link' + res.ChatId + '" data-transition="slide" href="#" onclick="ClickonChat(\'' + res.ChatId +
                          '\')"><p id=' + res.ChatId + ' class="Chat speechright msgRight fontcolorchat">' + res.BrokerMessage +
                        ...
                        $('#chat1').append('<a id="link' + res.ChatId + '" data-transition="slide" href="#" onclick="ClickonChat(\'' + res.ChatId +
                            '\')"><p id=' + res.ChatId + ' class="Chat speech fontcolorchat"><span style="padding-left: 2px;">' + res.CustomerMessage +
            ...
            success: function (data) {
                ...
                if (issuccess == true) {
                    ...
                    if (DeleteMsgId.length > 0) {
                        //alert("New - " + NewLoadingMessagesId + ", Delete - " + DeleteMsgId);
                ...
                success: function (data) {
                    ...
                    if (issuccess == true) {
                        ...
                        if (parseInt(SetUnreadArrayInterval.length) > 0) {
                            SetIsReadFlag(SetUnreadArrayInterval, BrokerId, "");
                        }
                        else {
                            GetChatFlag = 'Yes';
                        }
...
function SaveBrokerChat(chatcontent) {
    // alert("Called " + chatcontent + "," + webServiceUrl);
...
function DeleteChatMessages() {
    ...
    $.ajax({
        type: "POST",
        url: webServiceUrl,
        data: { UserId: BrokerId, MessageId: listtodelete, ActionName: "DoDeleteMultipleChatMessage" },
        success: function (data) {
