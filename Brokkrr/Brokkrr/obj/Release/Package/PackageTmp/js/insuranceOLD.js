...
var newPurchaseOrCurrentlyInsured, whenNeedInsurence, noOfEmployee, industry, contactInformation, coverageamount, siccode, grossrevenue, acquisitionIsInsured,
    typeOfAuto, valueOfHome, language, notes, insuranceCompany, typeofinsurance, planSize, CurrentPlan, howManyStalls, NoofLocation, grossrevenueforgarage, SubmitWithLogin, NumberofEmployees, GrossPayroll;
var colors;
                       ...
                       '<div class="question-div question-div1">' +
                           '<div class="next-question-div">' +
                               ...
                               '<div id="Life_Ins" class="col-md-2 col-sm-2 col-xs-6"><center><img class="next-question-button img-responsive" id="Life Insurance" src="../Images/NewAssets/Mlife.png" alt="Life Insurance" /></center><div style="text-align: center;margin-top: 7%;">Life</div></div>' +
                                ...
                                '<div id="Busi_Ins" class="col-md-2 col-sm-2 col-xs-6"><center><img class="next-question-button img-responsive" id="Business Insurance" src="../Images/NewAssets/Mbusiness.png" alt="Business Insurance" /></center><div style="text-align: center;margin-top: 7%;">Business</div></div>' +
...
function mainfunction(selectedId, mode) {
    ...
    if (selectedId.id == 'Auto Insurance') {
        $('#InsuranceOtherQuestion').append(typeOfAuto + newPurchaseOrCurrentlyInsured + language + notes + appendcontatcus);
        $parent.nextAll('.main').hide();//hide all main question 
        fillInsuranceCompanyList();
        IsLogin();
        fillLanguage();
    ...
    if (selectedId.id == 'Life Insurance') {
        $('#InsuranceOtherQuestion').append(coverageamount + language + notes + appendcontatcus);
        $parent.nextAll('.main').hide();//hide all main question 
        IsLogin();
        fillLanguage();
        $('.ui-igcombo-field').prop('readonly', true);
    ...
    if (selectedId.id == 'Business Insurance') {
        $('#InsuranceOtherQuestion').append(acquisitionIsInsured + grossrevenue + industry + siccode + language + notes + appendcontatcus);
        $parent.nextAll('.main').hide();//hide all main question 
        fillIndustry();
        IsLogin();
        fillLanguage();
    ...
    if (selectedId.id == 'Garage Keepers') {
        ...
        $('#InsuranceOtherQuestion').append(howManyStalls + NoofLocation + grossrevenueforgarage + appendcontatcus);
        $parent.nextAll('.main').hide();//hide all main question 
        //fillInsuranceCompanyList();
        IsLogin();
        fillLanguage();
    ...
    if (selectedId.id == '401k') {
        ...
        $('#InsuranceOtherQuestion').append(CurrentPlan + noOfEmployee + planSize + appendcontatcus);
        $parent.nextAll('.main').hide();//hide all main question 

        if (IsUserLogin == 'Authorize') {
            $('#spanPlanSize').removeClass('line');
        }
