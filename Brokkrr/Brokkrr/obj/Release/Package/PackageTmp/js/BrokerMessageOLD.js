...
//alert('Main Called');
// No Messages Found !
function GetBrokerMessages(UserId, WeServiceUrl) {


   // alert('Called GetBrokerMessages ' + UserId + ' WeServiceUrl ' + WeServiceUrl);
    ...
    $.ajax({
                      ...
                      //  alert(res);
                       ...
                       // alert('DB ' + dt);
                        ...
                        content = '<div id=' + res.MessageId + '  class="MessageList col-md-12 col-sm-12 col-xs-12 nopadding">'
                                + '<div style="display:none;" id="Hidden' + res.MessageId + '" class="CustomerName"> ' + res.CustomerName + ' </div>'
                            + '<div style="cursor: pointer;"  onclick="getChatMessageBroker(\'' + res.MessageId + '\',\'' + res.CustomerMsgId + '\' ,\'' + res.BrokerId + '\',\'' + res.CustomerId + '\',\'' + res.CustomerName + '\', \'' + res.Message + '\',\'' + msgdate + '\')">'
                           ...
                           + '<div class="col-md-5 col-sm-5 col-xs-5 MainMessage" id="LatestMsg' + res.MessageId + '">'
                             ...
                             + '</div></div>'
                              ...
                              + '<div class="col-md-12 col-sm-12 col-xs-12" ><span class="countmsg" id="Count' + res.MessageId + '" ></span></div>'
                                + '<div class="col-md-12 col-sm-12 col-xs-12"><span class="tickdiv"> <img src="../Images/Assets/redioButton.png" style="width:20px;height:20px;" id="tick' + res.MessageId + '" onclick="getBrokerDeletedItems(\'' + res.MessageId + '\')" class="tickimg"></span></div>'
                        ...
                        if (issuccess == true) {
                            ...
                            $.each(MessageDetails, function (i, res) {
                                ...
                                if (parseInt(res.Cnt) >= 1) {
                                  ...
                                  //  var totalunreadconver = "10";
                                   // jQuery('#Count' + res.MessageId).addClass('circlechatcountOnMessage');
                                  //  jQuery('#date' + res.MessageId).addClass('UnreadMsgdate');
                                  //  $('#Count' + res.MessageId).text(totalunreadconver);
                                ...
                                //var month = 0;
                                ////alert('now mnt ' + (nowdt.getMonth() + 1));

                                //if ((nowdt.getMonth() + 1) <= 9) {
                                //   // alert('if');
                                //    month = '0' + (nowdt.getMonth() + 1);
                                //}
                                //else {
                                //   // alert('else');
        ...
        error: function (e) {
            //alert('GetBrokerMessages error: ' + e.msg);
        }
    });
...
function getChatMessageBroker(BrokerMessageId, CustomerMessageId, BrokerId, CustomerId, CustomerName, Message, MsgDate) {
    deletemsgarrayBroker = [];
    ...
    $.ajax({
        ...
        success: function (Data) {
            ...
            window.location.href = "/Chat/BrokerChat?BrokerMessageId=" + Data.BrokerMessageId + "&CustomerMessageId=" + Data.CustomerMessageId + "&BrokerId=" + Data.BrokerId + "&CustomerId=" + Data.CustomerId + "&CustomerName=" + Data.CustomerName;
    ...
    //    window.location.href = "/Chat/BrokerChat?BrokerMessageId=" + BrokerMessageId + "&CustomerMessageId=" + CustomerMessageId + "&BrokerId=" +BrokerId + "&CustomerId=" + CustomerId + "&CustomerName=" + CustomerName;
...
//$('#deletemessages').off("click").on("click", function () {
    ...
    $("#deletemessagesconfirm").click(function (e) {
        var listtodelete = deletemsgarrayBroker.toString();
       // alert('before ' + JSON.stringify(messagelist) + 'length ' + messagelist.length);

       
        $.ajax({
            type: "POST",
            url: webserviceURL,
            data: { UserId: GlobleUserID, MessageId: listtodelete, ActionName: "DoDeleteMultipleMessage" },
            success: function (data) {
               // alert('data '+data);
                var obj = JSON.parse(data);
                var issuccess = obj.IsSuccess;
                ...
                if (issuccess == true) {
                    ...
                    for (i = 0; i <= deletemsgarrayBroker.length; i++) {
                      
                        $('#' + deletemsgarrayBroker[i]).remove();
...
function getBrokerDeletedItems(id) {
    ...
    deletemsgarrayBroker.push(id);
    if (parseInt(deletemsgarrayBroker.length) > 0) {

        if (deletemsgarrayBroker.indexOf(id) >= 0) {

            $('#tick' + id).attr('src', '../Images/Assets/radioBtn_checked.png');
            $("#deletemessages").hide();
            $("#deletemultiplemsg").show();

          //  alert('len1 ' + deletemsgarrayBroker.length);

            for (var i = 0; i < deletemsgarrayBroker.length; i++) {
                for (var j = 0; j < deletemsgarrayBroker.length; j++) {
                    if (i != j) {
                        if (deletemsgarrayBroker[i] == deletemsgarrayBroker[j]) {

                           // alert('remove fun ' + deletemsgarrayBroker[i]);
                            removeBrokerTick(deletemsgarrayBroker[i]);
...
function removeBrokerTick(removeid) {
  ...
  //  alert(deletemsgarrayBroker.length);
    deletemsgarrayBroker = jQuery.grep(deletemsgarrayBroker, function(value) {
...
MessageIntervalbroker=setInterval(function ()
	    ...
	    $.ajax({
	        type: "POST",
	        url: webserviceURL,
	        data: { UserId: GlobleUserID, TimeStamp: "", ActionName: "DoGetMessages" },
	        //contentType: "application/json; charset=utf-8",
	        //dataType: "json",
	        success: function (data) {
	            //alert('Success data '+ data);
      ...
      //  alert('hi');
			$.ajax({		
				type: "POST",
				url: webserviceURL,
				data: { UserId: GlobleUserID, ActionName: "DoGetUnreadMsgCount" },
				success: function (data) {	
					//alert('data '+data);
					var obj = JSON.parse(data);
					var issuccess = obj.IsSuccess;
					...
					if(issuccess==true)
					{
						...
						var profileimg='';
                            ...
                            + '<div class="col-md-2 col-sm-2 col-xs-2" style="text-align:left;padding-right:0;color:black;padding-top: 1%;">'
                                   + '<img class="circular--landscape circular--square circular--portrait" src=' + profileimg + ' id="profilepicprofile" style="position:none !important;" />'
                               + '</div>'
                           + '<div id="Name' + res.MessageId + '" class="col-md-3 col-sm-3 col-xs-3 CustomerName" style="text-align:left;padding-right:0;color:black;padding-top: 1%;">'
                                + res.CustomerName
                           + '</div>'
                           + '<div class="col-md-5 col-sm-5 MainMessage" id="LatestMsg' + res.MessageId + '">'
                              + res.Message
						...
						//alert(JSON.stringify(UnreadMsgArray));
