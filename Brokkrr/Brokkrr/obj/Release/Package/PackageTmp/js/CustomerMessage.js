                ...
                if (parseInt(ContactedMessageList.length) > 0) {
                    ...
                    $.each(ContactedMessageList, function (i, res) {
                        ...
                        if (messagelist.indexOf(res.MessageId) >= 0) {//alert('contains');
                         ...
                         + '<div class="col-md-10 col-sm-10 col-xs-10" style="cursor: pointer;padding-left: 0;padding-right: 0;"  href="#" onclick="getChatMessageCustomer(\'' + res.BrokerMsgId + '\',\'' + res.MessageId + '\' ,\'' + res.BrokerId + '\',\'' + res.CustomerId + '\',\'' + res.BrokerName + '\', \'' + res.Message + '\',\'' + msgdate + '\',\'' + profileimg + '\')"><div class="col-md-3 col-sm-3 col-xs-3">'
                            ...
                            + '</div>'
                             ...
                             + ' <div id="Name' + res.MessageId + '" class="col-md-12 col-sm-12 col-xs-12" style="font-weight:bold;font-size:20px;padding-left: 0;padding-right: 0;">' + res.BrokerName + '</div>'
...
function getChatMessageCustomer(BrokerMessageId, CustomerMessageId, BrokerId, CustomerId, BrokerName, Message, MsgDate, profileimg) {
    ...
    SetIsReadFlag("", CustomerId, CustomerMessageId);
    ...
    if (GlobalMsgClicked != CustomerMessageId) {
        ...
        $.ajax({
            type: "POST",
            url: webserviceURL,
            data: { BrokerId: BrokerId, CustomerId: CustomerId, BrokerMessageId: BrokerMessageId, CustomerMessageId: CustomerMessageId, TimeStamp: "", TableName: "CustomerMessages", ActionName: "DoGetMainMessage" },
...
MyVarBrokerMsg = setInterval(function () {
    $.ajax({
        type: "POST",
        url: webserviceURL,
        data: { UserId: TopCustomerId, TimeStamp: "", MessageId: TopCustomerMessageId, ActionName: "DoGetChatMessagesByMessageId" },
        success: function (data) {
...
MyVarBroker = setInterval(function () {
    ...
    $.ajax({
        type: "POST",
        url: webserviceURL,
        data: { UserId: TopCustomerId, MessageId: TopCustomerMessageId, ActionName: "DoGetUnreadChatMessages" },
        success: function (data) {
            ...
            if (issuccess == true) {
                ...
                if (parseInt(SetUnreadArrayInterval.length) > 0) {
                    ...
                    SetIsReadFlag(SetUnreadArrayInterval, TopCustomerId, "");
                }
                else {
                    // GetChatFlag = 'Yes';
                }
...
function SaveBrokerChat(chatcontent) {
    ...
    $.ajax({
        type: "POST",
        url: webserviceURL,
        data: { BrokerMsgId: TopBrokerMessageId, CustMsgId: TopCustomerMessageId, CustomerId: TopCustomerId, BrokerId: TopBrokerId, CustomerMessage: chatcontent, OldMessageId: 0, LocalDateTime: LocalDateTime, ActionName: "DoSaveCustomerChat" },
        success: function (data) {
            //alert('data '+data);
            var obj = JSON.parse(data);
            var issuccess = obj.IsSuccess;
            ...
            if (issuccess == true) {
                ...
                $.each(response, function (i, res) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: webserviceURL,
                        data: { message: message, title: title, msgcnt: msgcnt, UserId: TopBrokerId, ActionName: "DoSendNotification" },
                        success: function (data) {
                        },
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                        }
...
function DeleteChatMessages() {
    ...
    $.ajax({
        type: "POST",
        url: webserviceURL,
        data: { UserId: TopCustomerId, MessageId: listtodelete, ActionName: "DoDeleteMultipleChatMessage" },
        success: function (data) {
...
$(function () {
    ...
    $("#deletemultiplemessagesconfirm").click(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: webserviceURL,
            data: { UserId: TopCustomerId, MessageId: listtodeletemainmsg, ActionName: "DoDeleteMultipleMessage" },
            success: function (data) {
